...
A project directory named `java-first-code-refactoring` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-first-code-refactoring` directory which is created just now.
